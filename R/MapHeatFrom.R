#' Esta funcion crea un mapa de calor para cada jugador con sus zonas desde donde pasan
#'
#' @param df Un df
...
#' OptaMAPheatMapFrom(df,0)
#' @export
#'
#'
OptaMAPheatMapFrom <- function(df,home){
  ...
  for(i in players$player_id){
    ...
    p <- h +
             ...
             #Aquí creamos el mapa de calor
             stat_density2d(data=player_data,aes(x=x*100,y=y*70,fill = ..level..,alpha=..level..), geom="polygon",show.legend = FALSE) +
